...
class DeliveryPrice:
    ...
    def __init__(
        self, workload_rate: WorkloadRate, cargo: Cargo, distance: float
    ) -> None:
        self.workload_rate = workload_rate
        self.cargo = cargo
        self.distance = self.__validate_distance(distance)
        logger.info(
            f"Cargo is {self.cargo.__repr__()}. Workload rate is {workload_rate.value}. Distance is {distance}"
        )
    ...
    def calculate_price(self):
        if self.cargo.fragility and self.distance > 30:
            raise CalculatePriceException(
                "Can't deliver fragility cargo for distance more then 30km"
            )
